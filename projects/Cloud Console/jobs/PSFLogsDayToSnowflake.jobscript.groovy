...
import java.net.URL;//doesn't seem to be used
import java.util.regex.Matcher;//apparently not explicitly required
...
day = API.getProperty('vFrom'); 
LOG.info('From: '+day);
plus = 1;
acceptedFormat = "yyyy-MM-dd"; 
day = Date.parse(acceptedFormat, day); 
use(TimeCategory) 
{ 
  to_datum = day.plus(plus);
  nextday = to_datum.format(acceptedFormat);
  API.setProperty('vTo',nextday);
  LOG.info('To: '+nextday);
}
...
API.setProperty('vTableSnowflake','PSF');
