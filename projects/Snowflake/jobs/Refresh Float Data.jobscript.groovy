...
def idstring = id.toString()+'_${vFloatEndpoint}';
def job = 'Float_${vFloatEndpoint}';
...
def desc = 'Float_${vFloatEndpoint} refreshed at '+startfrm; // Please update this with a Brief Description of the job
def vMonth = '';
def cubes = 'Snowflake Float_${vFloatEndpoint}'; // Please define the cubes here that will be affected by this job (Refer to the cubes dimesnion to get the exact element name
...
// Update the section below to run your job with the required variables
	state = API.executeJob("Update Float Tables");
...
def idstring2 = id2.toString()+'_${vFloatEndpoint}';
