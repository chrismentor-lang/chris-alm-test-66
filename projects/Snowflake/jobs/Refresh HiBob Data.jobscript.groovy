...
def idstring = id.toString()+'_${vHiBobEndpoint}';
def job = 'HiBob_${vHiBobEndpoint}';
...
def desc = 'HiBob_${vHiBobEndpoint} refreshed at '+startfrm; 
def vMonth = '';
def cubes = 'Snowflake HiBob_${vHiBobEndpoint}'; 
def user = state.getUserName();
LOG.info('User: '+user);

IDatabase db = OLAP.getDatabase("cgLocal_statThinkerbell_jc"); 
...
// Update the section below to run your job with the required variables
	state = API.executeJob("Update Hi Bob Tables");
...
def idstring2 = id2.toString()+'_${vHiBobEndpoint}';
